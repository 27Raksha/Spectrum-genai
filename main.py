...
def process_video(video_file):
    ...
    selected_frames = []
    previous_frame = None
    threshold = 0.50 # Change threshold according to need
...
def get_gemini_response2(input,image,user_prompt):
...
def process_image(uploaded_file):
    if uploaded_file is not None:
        ...
        return text, len(text.split()), image
    return "", 0, None
                ...
                for uploade_file in uploaded_file:
                    if uploade_file.type.endswith('pdf'):
                        ...
                        if uploade_file is not None:
                            text, word_count, image = process_image(uploade_file)
                            threshold = 50 
                            if word_count > threshold:
                                ...
                                """
                                image_data=input_image_details(uploade_file)
                                response=get_gemini_response2(input_prompt,image_data,input_text)
                                st.subheader("Response:")
                                st.write(response)
                                # insert_question_answer(input_text, response, user_id)
                                audio_file = text_to_speech(response)
                                ...
                                # image2 = Image.open(uploade_file)
